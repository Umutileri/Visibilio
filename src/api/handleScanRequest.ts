...
export async function runScanSession(sessionId: string, url: string): Promise<void> {
  ...
  try {
    ...
    const artifacts = result.results.flatMap((scan) => scan.ok ? [{ id: sessionId + "_" + scan.viewport.width + "x" + scan.viewport.height, kind: "screenshot" as const, contentType: "image/png" as const, path: scan.screenshot.path, viewport: scan.viewport, capturedAt: scan.screenshot.capturedAt }] : []);
